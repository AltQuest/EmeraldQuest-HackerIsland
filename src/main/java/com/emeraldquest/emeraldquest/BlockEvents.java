...
public class BlockEvents implements Listener {
	...
	public BlockEvents(EmeraldQuest plugin) {
	...
	void onBlockCatchFire(BlockIgniteEvent event) {
		if(event.getCause().equals(IgniteCause.FLINT_AND_STEEL)) {
			if(event.getPlayer() != null) {
				if (!emeraldQuest.canBuild(event.getBlock().getLocation(), event.getPlayer())) {
    ...
    void onBlockBreak(BlockBreakEvent event) {
    	...
    	if(event.getBlock().getType().equals(Material.BEDROCK)||event.getBlock().getType().equals(Material.END_STONE_BRICKS)||event.getBlock().getType().equals(Material.END_STONE)) {
    		event.setCancelled(true);
    	// If player is in a no-build zone, cancel the event
    	} else if (!emeraldQuest.canBuild(event.getBlock().getLocation(), event.getPlayer())) {
			event.setCancelled(true);
			emeraldQuest.error(event.getPlayer(), "You may not break blocks here!");
        } else {
	...
	void onBlockPlace(BlockPlaceEvent event) {
		...
		if (!emeraldQuest.canBuild(event.getBlock().getLocation(), event.getPlayer())) {
			event.setCancelled(true);
			emeraldQuest.error(event.getPlayer(), "You may not place blocks here!");
		} else if(event.getBlock().getType().equals(Material.BEDROCK)) {
			emeraldQuest.error(event.getPlayer(), "Placing bedrock is not allowed!");
			event.setCancelled(true);
		} else {
			event.setCancelled(false);
		}
